...
#execute if entity @s[tag=loading_logo_load,scores={time=..50}] at @e[type=minecraft:armor_stand,tag=saved_player_pos] run tp @s ~ ~ ~ 0 0
#execute unless entity @e[type=item_display,tag=loading_logo_p,limit=1] run summon armor_stand ~ ~1 ~1 {Marker:1b,Invisible:1b,Invulnerable:1b,PersistenceRequired:1b,NoBasePlate:1b,NoGravity:1b,Tags:["loading_logo_p","markerignore"],ArmorItems:[{},{},{},{id:"netherite_shovel",Count:1b,tag:{CustomModelData:77,Unbreakable:1}}]}
...
#execute if entity @s[scores={time=50..}] run effect clear @s minecraft:blindness
#execute if entity @s[scores={time=50..}] run effect clear @s minecraft:night_vision
