...
execute positioned ^ ^ ^ if entity @e[type=minecraft:glow_item_frame,distance=..0.7,tag=hard_light_bridge] as @e[type=minecraft:area_effect_cloud,tag=sbpg.portal_first,tag=floor,tag=north] at @s positioned ^ ^1 ^-1 run function hard_light_bridge:create/y/raycast
