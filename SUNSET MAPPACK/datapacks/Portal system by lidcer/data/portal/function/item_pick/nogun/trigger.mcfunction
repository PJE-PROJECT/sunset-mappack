...
execute unless entity @e[tag=hovering,limit=1] positioned ^ ^0.5 ^1 unless entity @e[type=minecraft:item_display,distance=..1,tag=meg_field] positioned ^ ^ ^ if entity @e[distance=..1.5,tag=prop,tag=!hovering,tag=!hoverignore,limit=1] if block ~ ~ ~ #portal:hoverignore positioned ^ ^0.5 ^ if block ~ ~ ~ #portal:hoverignore positioned ^ ^ ^-1 if block ~ ~ ~ #portal:hoverignore positioned ^ ^ ^2 if block ~ ~ ~ #portal:hoverignore positioned ^ ^-0.5 ^-1 unless entity @e[type=minecraft:armor_stand,distance=..1,tag=button_pedestal_base,tag=!useless,tag=!activated] run tag @s add itempicker_activated
