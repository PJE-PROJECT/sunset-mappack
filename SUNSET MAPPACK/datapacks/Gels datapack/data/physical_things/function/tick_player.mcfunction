...
#tag @s[scores={physical_things.jump=1..}] add jump



#tag @s[scores={physical_things.rot2=..0}] remove jump
#scoreboard players set @s[scores={physical_things.jump=1..}] physical_things.jump 0

#execute if score @s[predicate=physical_things:guel_fill] physical_things.coas matches 1.. run function physical_things:tick_player

execute if entity @s[tag=gel_cooldown] run scoreboard players add player.gel.jump.cooldown buffer 1
execute if score player.gel.jump.cooldown buffer matches 15.. run tag @s remove gel_cooldown
execute if score player.gel.jump.cooldown buffer matches 15.. run scoreboard players set player.gel.jump.cooldown buffer 0


execute if entity @s[tag=!gel_stand_blue,nbt={OnGround:1b}] run function physical_things:gel_enter_blue
execute unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=blue_gel,tag=D,limit=1] if entity @s[tag=gel_stand_blue,nbt={OnGround:1b}] run playsound minecraft:player_exit_jump_paint_01 ambient @a ~ ~ ~ 0.2 1 0
execute unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=blue_gel,tag=D,limit=1] if entity @s[tag=gel_stand_blue,nbt={OnGround:1b}] run tag @s remove gel_stand_blue

execute if entity @s[tag=!gel_stand_orange,nbt={OnGround:1b}] run function physical_things:gel_enter_orange
execute unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=orange_gel,tag=D,limit=1] if entity @s[tag=gel_stand_orange,nbt={OnGround:1b}] run playsound minecraft:player_exit_speed_paint_01 ambient @a ~ ~ ~ 0.1 1 0
execute unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=orange_gel,tag=D,limit=1] if entity @s[tag=gel_stand_orange,nbt={OnGround:1b}] run attribute @s minecraft:generic.movement_speed base set 0.09
execute if entity @s[scores={shiftedit=0},tag=!gel_cooldown,nbt={OnGround:1b}] if entity @e[type=minecraft:item_frame,distance=..0.8,tag=orange_gel,tag=D,limit=1] if entity @e[type=minecraft:item_frame,distance=..0.8,tag=blue_gel,tag=D,limit=1] run function physical_things:player/end_falling
execute unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=orange_gel,tag=D,limit=1] if entity @s[tag=gel_stand_orange,nbt={OnGround:1b}] unless entity @e[type=minecraft:item_frame,distance=..0.8,tag=blue_gel,tag=D,limit=1] run tag @s remove gel_stand_orange
